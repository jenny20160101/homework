defmodule Puzzle5 do
  ...
  def row(boarding_pass) do
    ...
    %{start: start, end1: end1} = get_half_region_of_rows(chars_of_row, 0, range)
    start
  end

  def get_half_region_of_rows(chars_of_row, index, %{start: start, end1: end1} = range)
      when index <= 6 do
    range = get_half_region_of_row(Enum.at(chars_of_row, index), range)
    get_half_region_of_rows(chars_of_row, index + 1, range)
  end
  ...
  def get_half_region_of_row("B", %{start: start, end1: end1} = range) do
    new_start = start + Integer.floor_div(end1 - start, 2) + 1
    %{start: new_start, end1: end1}
  end

  def get_half_region_of_row("F", %{start: start, end1: end1} = range) do
    new_end = start + Integer.floor_div(end1 - start, 2)
    %{start: start, end1: new_end}
  end
